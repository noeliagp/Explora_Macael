...
<div class="container-fluid p-0">
    <div class="monumentos-section text-white text-center" style="background-image: url('https://i.ibb.co/X4y4br0/image.png'); background-size: cover; background-position: center;">
        ...
        <div class="container d-flex align-items-center justify-content-center" style="min-height: 50vh;">
            <div>
                <h1 class="display-4">{{ $Monumento->nombre }}</h1>
                <p class="lead">Descubre más sobre este monumento</p>
            </div>
        </div>
    </div>
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body">
                        <div class="row">
                                        ...
                                        <div class="carousel-inner rounded">
                                            @foreach($imagenes as $index => $imagen)
                                                <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                                                    <img src="{{ $imagen }}" class="d-block w-100" alt="Imagen de Monumento">
                                                </div>
                                            @endforeach
                                        </div>
                                        @if(count($imagenes) > 1)
                            ...
                            <div class="col-md-12">
                                <h2 class="card-title text-center">{{ $Monumento->nombre }}</h2>
                                <p class="card-text lead text-justify">{!! nl2br(e($Monumento->descripcion)) !!}</p>
                            </div>
